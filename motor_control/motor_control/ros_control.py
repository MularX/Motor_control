...
DISPLAY_REFRESH_RATE = 0.0714  # ~14 HZ 
...
class ROSNode(Node):
    def __init__(self):
        ...
        for motor_id, pins in self.MOTOR_PINS.items():
            ...
            self.get_logger().info(f"Motor {motor_id} on PWM {piny['PWM']} i DIR {pins['DIR']} initialized.")
    ...
    def check_timeout(self):
        ...
        if current_time - self.last_message_time > 0.5:  # stop trigga
            self.stop_all()
    ...
    def set_motor(self, motor_id, direction, speed_percent):
        ...
        duty_cycle = 100 - max(0, min(100, speed_percent))  
    ...
    def update_motors(self):
        ...
        speed = -100 * self.Vel_X + 100  # reversed logic
